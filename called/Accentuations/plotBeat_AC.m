...
function plotBeat_AC(data2load1, data2load2, FqOI, PosOI)

%% Load in data as provided in inputs

load(data2load1); % load(loadName)
load(data2load2) % load(saveName); data2load2 = saveName;
saveName = data2load2;
NPos = length(PosOI);
...
if ~exist('GA', 'var')
    for ss = 1:NSubj
        ...
        for l = 1:length(Labels)
            ...
            if ss == 1
                ...
                GA.(Labels{l}) = [GA.(Labels{l}); avgdata];
            end
    ...
    % Save GA
    save(saveName, 'Models', 'BinaryBeat', 'TripleBeat', 'CombBeat', 'OtherBeat', 'GA', 'Info2Display')
end
...
for l = 1:length(Labels)
    ...
    subplot(1,4,l); hold on
    %     plot(GA.(Labels{l})');
    plot(nanmean(GA.(Labels{l})), 'k','linewidth', 2)
    
    a = gca; Ypos = 0.9*a.YLim(2); Xpos = PosOI(2);
    text(Xpos, Ypos, sprintf('%d trials', length(GA.(Labels{l}))), 'fontweight', 'bold', 'fontsize', 12)
    a.XAxis.TickValues = PosOI;
    a.FontSize = 12;
    a.FontWeight = 'bold';
...
%% Focus on the Binary Beat: plots and stats

plotBinary_AC(GA.BinaryBeat,PosOI)

% Try with the no grouping
plotBinary_AC(GA.Other,PosOI)
