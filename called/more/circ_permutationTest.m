% [p, observeddifference, effectsize, p2Adjust] = permutationTest(sample1, sample2, permutations [, varargin])
%
...
%       p2Adjust - all pvalues from random permutations. Use then the
%       fdr_bh_pCorr function to retrieve the critical pvalue for
%       statistical testing and adjusted pvalues
%
% Usage example:
...
function [p, observeddifference, effectsize, p2Adjust] = circ_permutationTest(sample1, sample2, permutations, varargin)
...
observeddifference = circ_dist(circ_mean(sample1(~isnan(sample1))), circ_mean(sample2(~isnan(sample2))));
effectsize = observeddifference / nanmean([circ_std(sample1(~isnan(sample1))), circ_std(sample2(~isnan(sample2)))]);
...
allobservations = [sample1(~isnan(sample1)); sample2(~isnan(sample2))]; % avoid nans here
...
for n = 1:permutations
    ...
    permutation(1:length(sample1)) = []; % reduce vector
    randomSample2 = allobservations(permutation(1:length(randomSample1)));
    
    % saving differences between the two samples
    randomdifferences(1,n) = circ_dist(circ_mean(randomSample1'), circ_mean(randomSample2'));
    
    p2Adjust(n) = (length(find(circ_dist(randomSample1,randomSample2) == 1))+1)/(numel(randomSample1)+1);
end
