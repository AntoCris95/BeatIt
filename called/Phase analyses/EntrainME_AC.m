...
outdir2 = fullfile(outdir, 'Entrainment_Hilb');

if ~exist(outdir2)
    mkdir(outdir2)
end
dir2save = 'C:\Users\p70068941\OneDrive\Work\UM\BAND\Writing\Subjective Rhythmization\Humans - BeatIt\New Figures\Phase synch';
...
if ~exist(saveName3)
    for ff = 1:2:length(files)
        ...
        % Savename
        saveName = sprintf('hilbert %s Subj%d.mat', FqLab, n);
        saveName = fullfile(outdir2, saveName);

        if ~exist(saveName)

            % Load 2 sess for each subj
            load(fullfile(outdir1, files(ff).name));
            ...
            cfg.bpfreq = FqOI; % according to what's up: bp/lp/hp freq
            cfg.bpfiltord = 3;
            ...
            % And sess2
            load(fullfile(outdir1, files(ff+1).name));
            ...
            cfg = [];
            data = ft_appenddata(cfg, data1,data);
            ...
            % Move to 3D Data matrix
            Data = zeros([length(data.trial),length(indch),length(data.time{1})]);

            for tt = 1:length(data.trial)
                Data(tt,:,:) = data.trial{tt}(indch,:);
            end
            ...
            %     figure; plot(imag(Data)')
            %     figure; plot(angle(Data)')


            clear data; data = Data;
            save(saveName, 'data', 'Events')
        ...
        for chs = 1:size(data,2)
            ...
            fig = shadedErrorBar(Time, squeeze(nanmean(data(:,chs,:))), SEdata, 'lineprops','-b');  %'lineprops',
            ...
            %         h1 = histogram(MVLrand(:,chs), Nbins, 'facecolor', 'c'); box off;
            % a = gca; a.YAxis.TickValues (1) = [];
            ...
            h1 = raincloud_plot(MVL(:,chs), 'box_on', 1, 'alpha', 1, 'color', cb(12,:), ...
                'box_dodge', 1, 'box_dodge_amount', .15, 'dot_dodge_amount', .15, 'box_col_match', 1, 'density_type','ks', 'line_width', 2);
            h2 = raincloud_plot(MVLrand(:,chs), 'box_on', 1, 'alpha', 0.5, 'color', cb(6,:), ...
                'box_dodge', 1, 'box_dodge_amount', .35, 'dot_dodge_amount', .35, 'box_col_match', 1, 'density_type','ks', 'line_width', 2);

            a = gca; a.XLim = [0 1]; a.XAxis.TickValues = [0 .5 1]; a.YAxis.TickValues(a.YAxis.TickValues < 0) = [];
            AdjAxes_AC;

            if row == 1
                legend([h1{1}, h2{1}], 'pre-DEV', 'rand', 'Location', 'northeast', 'fontsize', 9); legend boxoff
...
if Plot
    ...
    Time = linspace(TimeSeq(1), TimeSeq(end), size(DATA,4));
    ...
    for chs = 1:size(DATA,3)
        ...
        subplot(1,items,1:4);
        fig = shadedErrorBar(Time, MEANdata, SEdata, 'lineprops','-b');  %'lineprops',
        hold on; box off
        AdjAxes_AC
        ...
        h2 = raincloud_plot(tmpMVLavg, 'box_on', 1, 'alpha', 1, 'color', cb(12,:), ...
            'box_dodge', 1, 'box_dodge_amount', .35, 'dot_dodge_amount', .35, 'box_col_match', 1, 'density_type','ks', 'line_width', 2);
        h1 = raincloud_plot(tmpMVLrand, 'box_on', 1, 'alpha', .5, 'color', cb(6,:), ...
            'box_dodge', 1, 'box_dodge_amount', .15, 'dot_dodge_amount', .15, 'box_col_match', 1, 'density_type','ks', 'line_width', 2);
        ...
        legend([h1{1}, h2{1}], 'rand', 'pre-DEV', 'location', 'northeast', 'fontsize', 9); legend boxoff
        title(sprintf('Mean Vector Length\nTrial-based'), 'fontsize', 12)
...
figure; set(gcf, 'name', 'MVL Pooling across subj and chs', 'color', 'white', 'Position', [670 254 551 540]);
...
a = gca; a.XLim = [0 1]; a.XAxis.TickValues = [0 .5 1]; a.YAxis.TickValues(a.YAxis.TickValues < 0) = [];
AdjAxes_AC; ylabel('Kernel Density')
